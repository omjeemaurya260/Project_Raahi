...
// ─── Shared delivery-time helpers ────────────────────────────────────────────
// Returns total delivery time in minutes using the same threshold as pricing.
function calculateDeliveryTime(distanceKm) {
  if (distanceKm <= 100) {
    return distanceKm * 5;   // 5 min / km
  } else {
    return distanceKm * 2;   // 2 min / km for the ENTIRE distance
  }
}


// Converts raw minutes into a human-readable string, e.g. "8h 20m" or "24h 58m".
...
// Returns a "Delivery by" label calculated from now + deliveryMinutes.
// Shows time + date if delivery spills into the next day (or beyond).
function formatDeliveryBy(distanceKm) {
  const totalMinutes = calculateDeliveryTime(distanceKm);
  const arrival = new Date(Date.now() + totalMinutes * 60 * 1000);
  const now = new Date();
  const timeStr = arrival.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true });
  // Show date only when delivery is not today
  const sameDay = arrival.toDateString() === now.toDateString();
  if (sameDay) return timeStr;
  const dateStr = arrival.toLocaleDateString('en-IN', { day: 'numeric', month: 'short' });
...
const INITIAL_NOTIFICATIONS = [
  { id: 1, emoji: '📦', type: 'delivery', title: 'New delivery matched',  message: 'Your parcel has been matched with a Raahi partner.', time: '2 min ago',  read: false, action: 'My Deliveries' },
  { id: 2, emoji: '🚚', type: 'transit',  title: 'Delivery in transit',     message: 'Parcel #RAH2048 is currently on the way.',           time: '10 min ago', read: false, action: 'Tracking'      },
  { id: 3, emoji: '🔐', type: 'otp',      title: 'Pickup OTP required',     message: 'Enter the pickup OTP to confirm parcel handover.',   time: '15 min ago', read: false, action: 'Tracking'      },
  { id: 4, emoji: '📍', type: 'update',   title: 'Delivery update',         message: 'Your Raahi partner is 2.4 km away.',                time: '20 min ago', read: false, action: 'Tracking'      },
  { id: 5, emoji: '💰', type: 'payment',  title: 'Payment released',        message: '₹85 has been released to your Raahi partner.',      time: '1 hour ago', read: true,  action: 'Wallet'        },
];

const SIMULATION_EVENTS = [
  { emoji: '📍', type: 'update', title: 'Partner nearby', message: 'Your Raahi partner is 1.2 km away from pickup point.', action: 'Tracking' },
  { emoji: '🔐', type: 'otp', title: 'Pickup OTP generated', message: 'Handover OTP is 4829. Share with partner at pickup.', action: 'Tracking' },
  { emoji: '🤝', type: 'partner', title: 'Partner accepted delivery', message: 'A verified Raahi accepted your delivery route.', action: 'Tracking' },
  { emoji: '📦', type: 'delivery', title: 'Parcel picked up', message: 'Partner collected parcel #RAH2048 from Rohini Sector 7.', action: 'Tracking' },
  { emoji: '🚚', type: 'transit', title: 'Parcel in transit', message: 'Parcel #RAH2048 is on its way to Connaught Place.', action: 'Tracking' },
  { emoji: '✅', type: 'completed', title: 'Delivery completed', message: 'Parcel reached destination successfully.', action: 'My Deliveries' },
  { emoji: '💰', type: 'payment', title: 'Payment released', message: '₹85 has been transferred to partner wallet.', action: 'Wallet' },
  { emoji: '⏱️', type: 'warning', title: 'Delivery overdue alert', message: 'Partner delayed by 6 min due to traffic at Ring Road.', action: 'Tracking' },
  { emoji: '🔄', type: 'recovery', title: 'Recovery required', message: 'Route updated to avoid congestion. ETA adjusted.', action: 'Tracking' },
];

// ─── NotificationCenter component ────────────────────────────────────────────
function NotificationCenter({ notifications, onMarkRead, onMarkAllRead, onNavigate, onClose, onSimulate, onClear }) {
    ...
    <div className="notif-panel" role="dialog" aria-label="Notifications">
      <div className="notif-header">
        <div className="notif-header-left">
          ...
          {unreadCount > 0 && <span className="notif-count-badge">{unreadCount} unread</span>}
        </div>
        <div className="notif-header-right">
          {unreadCount > 0 && (
            <button className="notif-mark-all" onClick={onMarkAllRead}>Mark all as read</button>
          )}
          <button className="notif-close" onClick={onClose} aria-label="Close notifications"><X size={15} /></button>
        </div>
      </div>
        ...
        {notifications.length === 0 ? (
          ...
          notifications.map(n => (
            <button
              ...
              onClick={() => { onMarkRead(n.id); if (n.action) onNavigate(n.action); }}
            >
              <span className="notif-emoji" aria-hidden="true">{n.emoji}</span>
      ...
      <div className="notif-footer">
        <button className="notif-simulate-btn" onClick={onSimulate} title="Simulate a new incoming event">
          <Sparkles size={12} /> Simulate event
        </button>
        {notifications.length > 0 && (
          <button className="notif-clear-btn" onClick={onClear} title="Clear all notifications to test empty state">
            Clear all
          </button>
        )}
      </div>
...
function App() {
  ...
  useEffect(() => {
    ...
    const handler = (e) => { if (e.key === 'Escape') setNotificationsOpen(false); };
  ...
  const simulateEvent = () => {
    const event = SIMULATION_EVENTS[simIndex % SIMULATION_EVENTS.length];
    setSimIndex(prev => prev + 1);
    addNotification(event);
    notify(`New notification: ${event.title}`);
  };

  const navigateTo = (section) => {
    setNotificationsOpen(false);
    if (section === 'Send a Parcel') { setShowSend(true); }
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        <header className="topbar"><button className="mobile-menu" onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Open navigation menu"><Menu size={20} /></button><div><p className="eyebrow">Saturday, 19 September 2026</p><h1>{view === 'Overview' ? `Good morning, ${account.split(' ')[0]}` : view}</h1></div>
          <div className="top-actions">
            {/* Bell + notification panel container */}
            <div className="notif-wrap" ref={notifRef}>
              <button
                className={notificationsOpen ? 'icon-button notification-active' : 'icon-button'}
      ...
      {showSend && (
        <SendModal
          ...
          onFind={() => {
            setShowSend(false);
            setView('Tracking');
            notify('3 compatible Raahis found');
            addNotification({
              emoji: '📦',
              type: 'delivery',
              title: 'New delivery matched',
              message: 'Partner Aman Sharma matched for your shared route.',
              action: 'Tracking'
            });
          }}
        />
